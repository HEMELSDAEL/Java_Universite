...
public class RentalAgency {
    ...
    /* HashMap with the clients and the vehicle of this agency*/
    private Map<Client, Vehicle> vehiclerented = new HashMap<Client, Vehicle>();
    ...
    */
    public RentalAgency() {
        ...
        this.vehiclerented = new HashMap<Client, Vehicle>();
    }
    ...
    */
    public void displaySelection(VehicleFilter filter) {
    	List<Vehicle> selectedVehicle = select(filter);
        for (Vehicle v : selectedVehicle){
    ...
    */
    public boolean hasRentedAVehicle(Client client){
        if(this.vehiclerented.containsKey((client))){
            return true;
        }else{
            return false;
        }
    ...
    */
    public boolean isRented(Vehicle v){
        if (this.vehiclerented.containsValue(v)){
            return true;
        }else{
            return false;
        }
    ...
    */
    public double rentVehicle(Client client, Vehicle v) throws UnknownVehicleException, IllegalStateException {
    	if (!this.theVehicles.contains(v)) {
            throw new UnknownVehicleException("Le véhicule n'existe pas dans l'agence.");
        }
        if (this.isRented(v)) {
            throw new IllegalStateException("Le véhicule est déjà loué par un client.");
        }
        if (this.hasRentedAVehicle(client)) {
            throw new IllegalStateException("Le client est déjà loueur d'un autre véhicule.");
        }
        try {
            double rentalPrice = v.getDailyPrice(); // Exemple de récupération du prix de location
            vehiclerented.put(client, v); // Ajoute la location à la map
            return rentalPrice;
        } catch (Exception e) {
            e.printStackTrace(); // Affichez la trace de la pile pour identifier la cause de l'erreur
            throw e; // Réémettez l'exception pour indiquer que quelque chose a mal tourné
        }
    }
    ...
    */
    public void returnVehicle(Client client){
    	if (this.vehiclerented.containsKey(client)) {
            this.vehiclerented.remove(client);
        } 
    }
    ...
    */
    public Collection<Vehicle> allRentedVehicles(){
    	return this.vehiclerented.values();
    }
